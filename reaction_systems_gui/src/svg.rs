...
impl Svg {
    pub(crate) fn parse_dot_string(dot_str: &str) -> Result<Svg, String> {
        ...
        // yes we convert back to string because the svg generated by layout is
        // mangled, the one generated by resvg is less so and can be converted
        // with batik into pdf that can be used inside latex
        let original = svg_tree.to_string(&Default::default());

        let (svg_size, svg_scale) = {
        ...
        resvg::render(&svg_tree,
                      tiny_skia::Transform::from_scale(svg_scale, svg_scale),
                      pixmap_mut);
    ...
    pub(crate) fn svg(&self) -> Result<Vec<u8>, String> {
        Ok(self.original.clone().into_bytes())
    }
}
